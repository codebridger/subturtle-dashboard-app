...
Helper to call an RPC as the user:
```bash
rpc(){ curl -s -w "\n%{http_code}" "${H[@]}" "$BASE/function/run" -H "authorization: $TOKEN" -d "$1"; }
# e.g. rpc '{"name":"getSubscriptionDetails","args":{}}'
```
...
The webhook maps customer→user via the `stripe_customer` collection. Create the
...
mongosh "$MONGO/subturtle_dev_user_content" --quiet --eval \
  'JSON.stringify(db.stripe_customer.findOne({user_id:"<USERID-as-string>"}))'
```
...
mongosh "$MONGO/subturtle_dev_user_content" --quiet --eval 'db.free_credit.updateOne({user_id:U},{$set:{allowed_save_words_used:199}})'
...
```bash
mongosh "$MONGO/subturtle_dev_user_content" --quiet --eval 'db.free_credit.updateOne({user_id:U},{$set:{allowed_lived_sessions_used:3}})'
...
```bash
mongosh "$MONGO/subturtle_dev_user_content" --quiet --eval 'db.subscription.updateOne({user_id:U,status:{$nin:["canceled","incomplete_expired"]}},[{$set:{credits_used:"$total_credits"}}])'
rpc '{"name":"create-text-session","args":{"instructions":"hi"}}'   # 400 AI_CREDIT_EXHAUSTED
```
(For free users seed `free_credit.credits_used = total_credits` instead.)
...
for (const s of subs.data) if (s.status!=='canceled') await stripe.subscriptions.del(s.id);
```
```bash
mongosh "$MONGO/subturtle_dev_user_content" --quiet --eval '
  db.subscription.deleteMany({user_id:U}); db.free_credit.deleteMany({user_id:U});
  db.usage.deleteMany({user_id:U}); db.stripe_customer.deleteMany({user_id:"<USERID>"});
  db.phrase.deleteMany({refId:"<USERID>"}); db.phrase_bundle.deleteMany({refId:"<USERID>"});'
mongosh "$MONGO/subturtle_dev_cms" --quiet --eval 'db.auths.deleteMany({email:/^e2e\+/})'
```
> RPC `clearSubscriptionAndFreemium {userId}` clears `subscription`+`free_credit`+`usage`
> but NOT the Stripe sub or `stripe_customer` — cancel the Stripe sub explicitly.
