 ...
 */
const checkDailyAllocationFn = defineFunction({
  name: "checkDailyAllocation",
  permissionTypes: ["user_access"],
  callback: async ({ userId }: { userId: string }) => {
    ...
    if (!dailyCredits) {
      ...
      // Replace insertOne with create method
      const createdRecord = await dailyCreditsCollection.create(
        newDailyCredits
      );
 ...
 */
const addCreditFn = defineFunction({
  name: "addCredit",
  permissionTypes: ["user_access"],
  callback: async ({
    userId,
    ...
    if (activeSubscription) {
      ...
      // Replace insertOne with create method
      updatedSubscription = await subscriptionsCollection.create(
        newSubscription
      );
      isNew = true;
 ...
 */
const recordConversationUsageFn = defineFunction({
 ...
 */
const recordTranslationUsageFn = defineFunction({
  name: "recordTranslationUsage",
  permissionTypes: ["user_access"],
  callback: async ({
    userId,
    characterCount,
 ...
 */
const checkAndUpdateExpiredSubscriptionsFn = defineFunction({
  name: "checkAndUpdateExpiredSubscriptions",
...
async function getMonthlyUsage(userId: string): Promise<number> {
  ...
  const usageCollection = getCollection(DATABASE, USAGE_COLLECTION);
  // Use Model.aggregate directly which returns an array
...
// Export the functions to be used by the module system
export const functions = [
  checkDailyAllocationFn,
  addCreditFn,
  recordConversationUsageFn,
  recordTranslationUsageFn,
  checkAndUpdateExpiredSubscriptionsFn,
];
