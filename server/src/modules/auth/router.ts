...
import { reply, getCollection, userManager } from "@modular-rest/server";
...
auth.get("/google/code-login", async (ctx) => {
	...
	if (!registeredUser) {
		try {
			...
			ctx.throw(500, error instanceof Error ? error.message : "Registration failed");
			return;
		}
	} else {
	...
	// update user profile
	await updateUserProfile({ refId: userId as string, gPicture: picture as string, name: name as string }, false);
