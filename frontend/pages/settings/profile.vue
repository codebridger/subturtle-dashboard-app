                ...
                <form @submit.prevent="handleSubmit" class="flex flex-col items-center p-4">
                    ...
                    <div class="mb-6">
                        <div class="group relative mx-auto h-24 w-24 md:h-32 md:w-32">
                            ...
                            <div
                                ...
                                <svg class="h-6 w-6 text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100"
                    ...
                    <div class="w-full">
                        ...
                        <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <Input :label="t('profile.full-name')" v-model="name" type="text"
                                :placeholder="t('profile.full-name')" required :disabled="isSubmitting" />
                            <Input :label="t('profile.email')" :model-value="email" type="email"
                                :placeholder="t('profile.email')" required disabled />
                        </div>
                        ...
                        <div class="border-t border-gray-200 pt-4">
                            <div class="flex justify-end">
                                <Button :label="t('save-changes')" type="submit" size="lg"
                                    :shadow="!(isSubmitting || isUploading || !hasChanges)" color="primary"
                                    :loading="isSubmitting" :disabled="isSubmitting || isUploading || !hasChanges" />
                            </div>
                        </div>
                    </div>
                </form>
...
const hasChanges = computed(() => {
    ...
    return nameChanged || timezoneChanged || preferencesChanged || fileChanged;
});
