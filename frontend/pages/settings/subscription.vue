            ...
            <div v-if="activeSubscriptionData">
                <div class="flex flex-col gap-4">
                    <div class="flex items-center justify-between">
                        <p class="text-gray-600">{{ t('subscription.started-at') }} {{ new Date(activeSubscriptionData.start_date).toLocaleDateString() }}</p>
                    ...
                    <ul class="space-y-2.5">
                        ...
                        <li class="flex items-start">
                            ...
                            <span class="text-sm text-gray-700"
                                >Created at: {{ activeSubscriptionData.createdAt ? new Date(activeSubscriptionData.createdAt).toLocaleDateString() : '' }}</span
                            >
                        </li>
                        <li class="flex items-start">
                            <Icon name="IconCheck" class="mr-2 mt-0.5 h-4 w-4 flex-shrink-0 text-green-500" />
                            <span class="text-sm text-gray-700"
                                >Updated at: {{ activeSubscriptionData.updatedAt ? new Date(activeSubscriptionData.updatedAt).toLocaleDateString() : '' }}</span
                            >
                        </li>
                        <li class="flex items-start">
                            <Icon name="IconCheck" class="mr-2 mt-0.5 h-4 w-4 flex-shrink-0 text-green-500" />
                            <span class="text-sm text-gray-700"
                                >End date: {{ activeSubscriptionData.end_date ? new Date(activeSubscriptionData.end_date).toLocaleDateString() : '' }}</span
                            >
                        </li>
                    </ul>
                </div>
                ...
                >
                    ...
                    <div class="flex flex-grow flex-col p-2">
                        ...
                        <div class="mt-auto">
                            ...
                            <template v-if="!plan.is_freemium">
                                <Button v-if="!activeSubscriptionData" @click="initiateCheckout(plan.product_id)" color="primary" block>
