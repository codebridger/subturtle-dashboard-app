<template>
    <StModal :title="t('subscription.checkout.title', { plan: planName })" @close="$emit('close')">
        <div v-if="loading" class="py-10 text-center text-st-sm font-semibold text-st-muted">{{ t('subscription.checkout.loading') }}</div>

        <p v-else-if="error" class="rounded-st-md bg-st-danger-soft px-4 py-3 text-st-sm font-semibold text-st-red-600">{{ error }}</p>

        <div v-else>
            <!-- Localized price (EUR for EU visitors via Adaptive Pricing) -->
            <p class="mb-4 font-st-display text-st-xl font-black tracking-st-tight text-st-strong">
                {{ localizedTotal }}
                <span class="text-st-sm font-bold text-st-muted"
                    >/ {{ cadence === 'annual' ? t('subscription.pricing.year') : t('subscription.pricing.month') }}</span
                >
            </p>
            ...
            <StButton block color="primary" size="lg" :disabled="paying" @click="pay">{{ t('subscription.checkout.pay') }}</StButton>
            <p v-if="payError" class="mt-2 text-center text-st-sm font-semibold text-st-red-600">{{ payError }}</p>
        </div>
    </StModal>
</template>
...
<script setup lang="ts">
    ...
    import { StButton, StModal } from 'subturtle-ui';
    ...
    function refreshTotal() {
        try {
            ...
            localizedTotal.value = li?.unitAmount?.amount || session?.total?.total?.amount || '';
        } catch {
            /* ignore */
        }
    }
