<template>
    <!--
        ...
        off its left edge. It shows a signed-out visitor what they are signing in to.

        aria-hidden and pointer-events-none — none of this copy is real, so it must not reach a
        screen reader or take focus. Hidden below lg, where the sign-in column takes the screen.
    -->
    <div
        aria-hidden="true"
        class="pointer-events-none absolute -right-[90px] top-1/2 z-[1] hidden h-[560px] w-[780px] -translate-y-1/2 [perspective:1400px] lg:block"
    >
        <div class="absolute inset-0 origin-left [transform:rotateY(-19deg)_rotateX(5deg)_rotate(-2deg)]">
            ...
            >
                ...
                <div>
                    <div class="mb-3 text-st-2xs font-extrabold uppercase tracking-st-caps text-st-faint">
                        {{ t('auth.preview.due-today') }}
                    </div>
        ...
        <!-- Phrase cards, in front of the board and unblurred, so the eye lands on them first. -->
